...
def process_results(doc, completion, answer, invalid_outputs):
    ...
    if len(split_ans) > 1:
        ...
        if is_equiv(extract_ans, answer):
            return True, True  # Correct, Valid
        else:
            return False, True
    else:
...
def generate_code_task_prompt(input_text, model_name="llama-2-13b-code-alpaca"):
...
if __name__ == '__main__':
    ...
    # print(generate_code_task_prompt(instruction))
    problem_prompt = get_math_task_prompt()
    print(problem_prompt.format(instruction="<><><><><>"))