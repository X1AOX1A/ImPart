...
def cal_overall_sparsity_ratio(attn_uv_sparsity_ratio, mlp_uv_sparsity_ratio,
    ...
    nominator = 4*hidden_size*hidden_size*2*(1-attn_uv_sparsity_ratio) + \
                3*intermediate_size*hidden_size*(1-mlp_uv_sparsity_ratio) + \
                3*hidden_size*hidden_size*(1-mlp_uv_sparsity_ratio)
    if include_s:
        nominator += 4*hidden_size*(1-attn_uv_sparsity_ratio) + 3*hidden_size*(1-mlp_uv_sparsity_ratio)
...
def main(args):
    ...
    for weight_type in args.weight_types:
        for layer_num in range(args.layer_num):
            ...
            # cal sparsity ratios
            target_uv_sparsity_ratio = args.attn_uv_sparsity_ratio if "self_attn" in weight_type else args.mlp_uv_sparsity_ratio
            preprune_ratio = args.attn_preprune_ratio if "self_attn" in weight_type else args.mlp_preprune_ratio
            if args.C == "only_lowrank":
                dim = int(n * (1-preprune_ratio))
            else:
            ...
            # log
            if args.C == "only_lowrank":
                actual_uv_sparsity_ratio = 1 - dim / n
            else:
                actual_uv_sparsity_ratio = sum(sparsity_ratio_list+[1]*(n-dim)) / n
            assert math.isclose(actual_uv_sparsity_ratio, target_uv_sparsity_ratio, abs_tol=1e-3)
            dataframe.append([weight_name+".U", list(u.shape), preprune_ratio, actual_uv_sparsity_ratio, target_uv_sparsity_ratio])
            logger.info(f"{weight_name}.U | shape: {list(u.shape)} | preprune_ratio: {round(preprune_ratio, 4)} "
                      + f"| actual_uv_sparsity_ratio: {round(actual_uv_sparsity_ratio, 4)} | target_uv_sparsity_ratio: {round(target_uv_sparsity_ratio, 4)}")

    df = pd.DataFrame(dataframe, columns=["weight_name", "shape", "preprune_ratio", "actual_uv_sparsity_ratio", "target_uv_sparsity_ratio"])
    df.to_csv(os.path.join(args.log_dir, "sparsity_ratio.csv"), index=False)
...
class DebugConfig(argparse.Namespace):
    ...
    # mask args
    compression_ratio = 32  # alpha = 1-1/CR
    preprune_ratio = 0.60   # beta, none if C=="only_lowrank"
...
if __name__ == "__main__":
    ...
    logger = setup_logger(os.path.join(args.log_dir, "sparsify.log"))

    # update sparsity ratio for U and V in attn and mlp
    args.overall_sparsity_ratio =  1 - 1 / args.compression_ratio
    args.attn_uv_sparsity_ratio = (1+args.overall_sparsity_ratio) / 2
    args.mlp_uv_sparsity_ratio = (0.55 + 1.45*args.overall_sparsity_ratio) / 2

    if args.C == "only_lowrank":
        assert getattr(args, "preprune_ratio") is None
        args.attn_preprune_ratio = (1+args.overall_sparsity_ratio) / 2
        args.mlp_preprune_ratio = (0.55 + 1.45*args.overall_sparsity_ratio) / 2
    else:
    ...
    _overall_sparsity_ratio = cal_overall_sparsity_ratio(
        args.attn_uv_sparsity_ratio, args.mlp_uv_sparsity_ratio,
        hidden_size=hidden_size, intermediate_size=intermediate_size)
    import math
    assert math.isclose(_overall_sparsity_ratio, args.overall_sparsity_ratio, abs_tol=1e-3), \
        f"Overall sparsity ratio not match: Target ({args.overall_sparsity_ratio}) vs  Actual ({_overall_sparsity_ratio})"
